...
> **【済 / 着手不要】** (a) は本編作業 (調整キー書き戻し対応) で実装済み。
> (b) も実装済みだが、調整レイヤー / SoCo / GdFl は追加情報キーとして書き戻される
> ようになったため、警告対象はレイヤー効果 (lfx2 Color Overlay) のみに縮小済み。
> 本文中の `CountUnsupportedForPsdExport` の判定条件は当時のもので現状とは異なる。
