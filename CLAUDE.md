...
1. **Core/Parser/** (static, `PSDParser.Parse` is the sole public entry point) — reads the PSD binary via **Core/BigEndianBinaryReader.cs** (PSD is big-endian; all multi-byte reads are byte-swapped). `PSDHeaderReader` parses the header; `PSDLayerRecordParser` reads layer records and delegates additional-info blocks (`lsct` groups, adjustment keys `brit`/`hue2`/`levl`/`curv`/`thrs`/`post`/`blnc`/`grdm`/`nvrt` — `levl`/`curv` including per-channel R/G/B data, `hue2` including the colorize flag —, descriptor-format `SoCo`/`CgEd`/`GdFl` fills, `clbl` clipping flag, `lrFX`/`lfx2` color overlay, and the tool's own `dPSE` clip-adjustment marker) to `PSDAdditionalInfoParser` (which uses `PSDDescriptorParser` for the descriptor-format blocks); `PSDChannelDecoder` decompresses pixel data (Raw/RLE/ZIP); `PSDLayerAssembler` builds per-layer `Texture2D`s, assembles the flat layer list into a tree (`BuildLayerTree` — index 0 = bottom-most layer), sets initial `UI*` state, and folds `dPSE`-marked clip adjustment layers back into the base pixel layer's `UI*` fields (`FoldBackToolAdjustmentClips` — the inverse of export; markers that were masked/hidden/reblended externally are kept as regular layers). Dependencies flow one way: `PSDParser` → record/merged-image parsers → decoder/assembler/descriptor/blend-mode helpers (no cycles).
