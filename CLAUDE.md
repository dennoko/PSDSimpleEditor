...
1. **Core/Parser/** (static, `PSDParser.Parse` is the sole public entry point) — reads the PSD binary via **Core/BigEndianBinaryReader.cs** (PSD is big-endian; all multi-byte reads are byte-swapped). `PSDHeaderReader` parses the header; `PSDLayerRecordParser` reads layer records and delegates additional-info blocks (`lsct` groups, adjustment keys `brit`/`hue2`/`levl`/`curv`/`thrs`/`post`/`blnc`/`grdm`/`nvrt` — `levl`/`curv` including per-channel R/G/B data —, descriptor-format `SoCo`/`CgEd`/`GdFl` fills, `clbl` clipping flag, `lrFX`/`lfx2` color overlay) to `PSDAdditionalInfoParser` (which uses `PSDDescriptorParser` for the descriptor-format blocks); `PSDChannelDecoder` decompresses pixel data (Raw/RLE/ZIP); `PSDLayerAssembler` builds per-layer `Texture2D`s and assembles the flat layer list into a tree (`BuildLayerTree` — index 0 = bottom-most layer) and sets initial `UI*` state. Dependencies flow one way: `PSDParser` → record/merged-image parsers → decoder/assembler/descriptor/blend-mode helpers (no cycles).
...
6. **Editor/Writer/** (static, `PSDWriter.Save` is the sole public entry point) — the inverse of Core/Parser/: `PSDExportRecordBuilder` flattens the layer tree back into records (baking non-destructive adjustments on pixel layers via `LayerCompositor.RenderLayerForExport`; zero-area adjustment / SoCo / GdFl layers instead get their UI-edited values written back as additional-info blocks built by `PSDAdjustmentInfoWriter`, so they round-trip to Photoshop), `PSDLayerRecordWriter` serializes those records (including `ExtraBlocks` and the `clbl` flag), and `PSDPixelEncoder` handles RLE (PackBits) compression and texture read-back. The only export data loss left is layer effects (lfx2 Color Overlay), which the PSD-export dialog warns about.
